...
// Fetch all sellers
$sellersResult = mysqli_query($conn, "
    SELECT 
        user_id, username, email, phone_number, created_at, is_active, image, firstname, middlename, lastname, birthdate, address,
        (SELECT COUNT(*) FROM products WHERE seller_id = users.user_id) AS product_count
    FROM users 
    WHERE role = 'seller' 
...
$sellers = [];
if ($sellersResult) {
    while ($row = mysqli_fetch_assoc($sellersResult)) {
        $sellers[] = $row;
    }
}
...
<body>
        ...
        <section>
            <div class="section-wrap">
                <div class="top-container">
                    <h1 class="dasboard-header">Seller Accounts</h1>
                </div>
                        ...
                        <table class="users-table">
                            ...
                            <tbody>
                                <?php if (empty($sellers)): ?>
                                    <tr>
                                        <td colspan="7" style="text-align:center;">No seller accounts found.</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($sellers as $seller): ?>
                                        <tr data-user_id="<?= htmlspecialchars($seller['user_id']) ?>">
                                            <td><?= htmlspecialchars($seller['user_id']) ?></td>
                                            <td><?= htmlspecialchars(date('Y-m-d', strtotime($seller['created_at']))) ?></td>
                                            <td><?= htmlspecialchars($seller['username']) ?></td>
                                            <td><?= htmlspecialchars($seller['email']) ?></td>
                                            <td><?= htmlspecialchars($seller['phone_number']) ?></td>
                                            <td style="text-align: center;"><?= intval($seller['product_count']) ?></td>
                                            <td class="state-cell">
                                            ...
                                            <td style="text-align:center;">
                                                <a href="#"
                                                    class="btn btn-view view-seller-btn" style="margin-bottom: 10px;"
                                                    data-user_id="<?= htmlspecialchars($seller['user_id']) ?>"
                                                    data-username="<?= htmlspecialchars($seller['username']) ?>"
                                                    data-email="<?= htmlspecialchars($seller['email']) ?>"
                                                    data-phone="<?= htmlspecialchars($seller['phone_number']) ?>"
                                                    data-created="<?= htmlspecialchars(date('Y-m-d', strtotime($seller['created_at']))) ?>"
                                                    data-state="<?= $seller['is_active'] ? 'Active' : 'Suspended' ?>"
                                                    data-profile="<?= !empty($seller['image']) ? '../../images/profiles/' . htmlspecialchars($seller['image']) : '../../images/profile-circle-svgrepo-com.png' ?>"
                                                    data-firstname="<?= htmlspecialchars($seller['firstname']) ?>"
                                                    data-middlename="<?= htmlspecialchars($seller['middlename']) ?>"
                                                    data-lastname="<?= htmlspecialchars($seller['lastname']) ?>"
                                                    data-birthdate="<?= htmlspecialchars($seller['birthdate']) ?>"
                                                    data-address="<?= htmlspecialchars($seller['address']) ?>"
                                                    data-products="<?= intval($seller['product_count']) ?>">
                                                    View
                                                </a>
                                                <?php if (isset($seller['is_active']) && !$seller['is_active']): ?>
                                                    <a href="account-state.php?id=<?= $seller['user_id'] ?>&role=seller&action=reactivate"
                                                        class="btn btn-reactivate reactivate-btn">
                                                        Reactivate
                                                    </a>
                                                <?php else: ?>
                                                    <a href="account-state.php?id=<?= $seller['user_id'] ?>&role=seller&action=suspend"
    ...
    <script>
        document.querySelectorAll('.view-seller-btn').forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                ...
                document.getElementById('sellerModalProducts').textContent = btn.dataset.products;
